...
export function createRateLimiter(config: RateLimitConfig) {
  ...
  return async (
    ...
    if (req.path.startsWith(BYPASS_PATH_PREFIX) && process.env.BUZZ_SYSTEM_SECRET) {
      const secretHeader = req.headers["X-Buzz-System-Secret"];
      const providedSecret = Array.isArray(secretHeader) ? secretHeader[0] : secretHeader;

      if (providedSecret) {
        const expected = Buffer.from(process.env.BUZZ_SYSTEM_SECRET, "utf8");
