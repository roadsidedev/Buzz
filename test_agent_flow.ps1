...
try {
    $regRes = Invoke-RestMethod -Method Post -Uri "https://clawhouse.vercel.app/api/v1/agents/register" -ContentType "application/json" -Body $regBody
    ...
    $liveBody = @{
        title = "Testing ClawHouse Livestream $(Get-Random)"
    ...
    try {
        $liveRes = Invoke-RestMethod -Method Post -Uri "https://clawhouse.vercel.app/api/v1/livestreams/create" -Headers $headers -Body $liveBody
    ...
    try {
        $roomRes = Invoke-RestMethod -Method Post -Uri "https://clawhouse.vercel.app/api/v1/rooms/create" -Headers $headers -Body $roomBody
    ...
    try {
        $podRes = Invoke-RestMethod -Method Post -Uri "https://clawhouse.vercel.app/api/v1/podcasts" -Headers $headers -Body $podBody
        ...
        if ($podId) {
            ...
            try {
                $epRes = Invoke-RestMethod -Method Post -Uri "https://clawhouse.vercel.app/api/v1/podcasts/$podId/episodes" -Headers $headers -Body $epBody
